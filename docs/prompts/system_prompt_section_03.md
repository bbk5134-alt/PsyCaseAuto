...
5. **서술체 통일 (Fix-경어체)**
   - narrative 문체는 `'~하였다'`, `'~보였다'` 서술체로 통일
   - `'~하였습니다'`, `'~되었습니다'` 등 경어체 절대 금지
...
<!-- 수정: Step 5-7 P-4 정보제공자 누락 방지 / Step 5-8 P-4v2 full_text 참조 추가 -->
- [ ] **STT 전체 면담 텍스트(full_text 또는 segments)에 나타난 모든 정보 제공자 단락 생성 (누락 금지)**
  - STT의 `full_text`, `speaker_id`, `segments` 중 **어느 위치에 발언이 있든** 발언이 있는 정보제공자를 빠짐없이 포함
  - `segments`가 비어 있거나 없는 경우: `full_text` 전체 텍스트에서 `=== [환자 면담] ===`, `=== [환모 면담] ===`, `=== [언니 면담] ===` 등의 마커를 기준으로 발언자를 식별
  - 단락 작성이 어려운 경우에도 최소 형식 허용: "면담에 동석하여 [내용] 진술하였으며 면담에 협조적이었다. -- Reliable"
  - 환자/환모/언니/남편 등 — full_text 또는 segments에 등장하는 모든 정보제공자 포함 필수
- [ ] 각 단락: 정보 제공자명 굵게(`**`)
- [ ] 각 단락: 서술형 평가 최소 2문장

...
> ⚠️ **첫 번째 경과 기록 포함 모든 노트에 날짜 헤더 필수 (절대 규칙)**
> - 면담이 몇 개든, 첫 번째 노트라도 예외 없이 헤더로 시작
> - 헤더 없이 `S)`로 바로 시작하는 것 **절대 금지**
> - 올바른 예: `**2026. 03. 20. (OPD #1)**` → 줄바꿈 → `S) ...`
> - 잘못된 예: `S) 6개월째 잠을 거의...` (헤더 누락 — 규칙 위반)

...
- 이미지 아래 간단한 설명 산문 1~2문장: 시계열 변화 요약 ("입원 시 기분 점수 N점에서 N일차 N점으로 변화" 등)
...
**수치 추출 규칙**:
1. 면담 기록에 명시적 수치(BDI, BAI, PHQ-9 등)가 있으면 해당 점수 참조하여 mood_score 산정
2. 명시적 수치 없는 경우: 환자의 서술적 표현에서 임상적으로 추정
   - "죽고 싶다", "기생충 같다" → 2-3점
   - "많이 힘들다", "우울하다" → 3-4점
   - "조금 나아졌다" → 5-6점
   - "안정됐다" → 6-7점
3. 추정 수치는 반드시 `"estimated": true` 표시
...
{
  "mood_chart": {
    "narrative": "[기분 차트 이미지 삽입]\n\n입원 시 scale: BDI N / BAI N / MDQ N개\n\n입원 초기 기분 점수 N점(HD#N)에서 퇴원 시 N점(HD#N)으로 변화하였다.",
    "structured": {

...
> **변경 이력**: v1.0(2026-03-27) → v2.0(2026-03-29) → v3.0(2026-04-01) → v3.1(2026-04-01, 아키텍처 감사 반영) → v3.1 rev.2(2026-04-01, 6단계 평가 반영) → **v3.1 rev.3(2026-04-03, §18 운용 문서 보완)**
...
> **rev.3 추가**: §18 Halluc 경고 대응 절차 + 모바일 수정 한계 + 최소 체크리스트 추가. §19-3 해당 항목 완료 처리.
...
3. **검토/수정**:
   - ⚠️ 표시 섹션(S11, S12) 집중
   - Halluc issue 항목 확인 (아래 "Halluc 경고 대응" 참조)
   - 날짜/약물명/용량 교차 확인
   - 진단명 최종 확정
4. **내보내기**: Docs → 파일 → 다운로드 → .docx (또는 직접 인쇄)
5. **저장**: `reports/final_YYYYMMDD.docx` — draft_ 파일은 절대 수정 금지

### Halluc 경고 대응 (v3.1 rev.3 추가)

Telegram 완성 알림에 Halluc issue 목록이 포함된 경우 다음 순서로 처리:

| 우선순위 | 이슈 유형 | 처리 방법 |
|---------|----------|----------|
| 1 | `fabricated_fact` (사실 날조) | 해당 문장 **삭제** 또는 STT 원문으로 교체 |
| 2 | `inference_unmarked` (추론 미표기) | 문장 끝에 `[추론]` 표시 추가 후 유지 |
| 3 | `severity_distortion` (심각도 왜곡) | STT 원문 수치/표현으로 교체 |
| 4 | `informant_error` (정보 제공자 오류) | 발언자 표기 수정 (환자↔환모 등) |

**시간 없을 때 최소 처리 (발표 직전)**: `fabricated_fact` 항목만 처리 + `[확인 필요]` 표시 후 발표. `inference_unmarked`는 발표 후 수정.

> ⚠️ Halluc check는 `context_truncated: true`인 경우 일부 섹션만 검증됨. 탐지 안 된 섹션도 임상 판단으로 검토 필요.

### 모바일 수정 한계 (v3.1 rev.3 추가)

| 환경 | 권장 작업 | 비권장 |
|------|----------|--------|
| 모바일 (Docs 앱) | Halluc 경고 확인, 진단명 검토, fabricated_fact 삭제 | 12섹션 전체 산문 수정, 표 편집 |
| PC (Chrome/Docs) | 전체 검토/수정, 서식 정리, .docx 내보내기 | — |

**모바일 최소 체크리스트**:
- [ ] HIGH SUICIDE RISK 배너 표시 여부
- [ ] 진단명 (Axis I) 정확성
- [ ] 약물명/용량 오류
- [ ] fabricated_fact 이슈 처리
...
| **Halluc 경고 대응 부재** | 보고서 받고 발표 급한데 Halluc 경고 시 행동 지침 없음 | ✅ §18 "Halluc 경고 대응" 절 추가 완료 (v3.1 rev.3) |
| **모바일 보고서 수정 한계** | 12섹션 분량을 모바일 Docs에서 수정은 비현실적 | ✅ §18 "모바일 수정 한계" 절 + 최소 체크리스트 추가 완료 (v3.1 rev.3) |

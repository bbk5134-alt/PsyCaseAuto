...
> **작성일**: 2026-04-05 | **최종 업데이트**: 2026-04-06 (세션 40)
...
## Claude.ai vs Claude Code 역할 분리 가이드 (Tier 5~)

### Step별 역할 판단

| Step | 주요 작업 | Claude.ai 사전 작업 | 이유 |
|------|----------|:-------------------:|------|
| **5-0** | QC 프롬프트 v2.1 수정 | ❌ 불필요 | 단순 파일 수정, Claude Code 직접 처리 |
| **5-1** | GS2 구조 분석 + 갭 목록 | ❌ 불필요 | Claude Code가 파일 직접 읽고 비교 → 더 정확 |
| **5-2** | Mock 면담 JSON (초진) 구어체 생성 | ✅ 필수 | 자연스러운 구어체·대화 흐름은 Claude.ai 우수, GS2 docx 통째로 첨부 가능 |
| **5-3** | Mock 면담 JSON (경과) | ✅ 필수 | 동일 이유 |
| **5-4** | 여자친구 임상 검수 | ❌ 해당 없음 | 사용자 주도 |
| **5-5** | WF2 E2E 실행 | ❌ 불필요 | n8n 조작은 Claude Code만 가능 |
| **5-6** | QC 채점 | ✅ 필수 | Claude.ai 주역할 (이미 명시) |
| **5-7** | 프롬프트 수정 설계 | ✅ 필수 | Claude.ai 주역할 (이미 명시) |
| **5-8** | regression framework 문서화 | ❌ 불필요 | 단순 문서화·Pin 정리 |

> **요약**: Claude Code 직접 → 5-0, 5-1, 5-5, 5-8 / Claude.ai → Claude Code → 5-2, 5-3 / Claude.ai 주역할 → 5-6, 5-7

---

### Claude.ai 요청 시 Step별 첨부 파일

#### Step 5-2 / 5-3 — Mock 면담 JSON 생성

| 구분 | 첨부물 | 용도 |
|------|--------|------|
| **필수** | GS2 보고서 전문 (`260401_Case conference_R1_문혜린.docx`) | 역공학 원본 |
| **필수** | `stt_20260320.json` (PT-001 초진) | JSON 구조 참조 |
| **필수** | milestone Step 5-2 Task 발췌 (5-2a~5-2f) | 요구사항 전달 |
| 선택 | `stt_20260328.json` (PT-001 경과) | Step 5-3 시 구조 참조 |

> **핵심 지시사항 반드시 포함**: "보고서 문장 그대로 복사 금지, 노이즈 5~10% 추가, Manic circumstantiality 반영, 3명 정보원 분리"

#### Step 5-6 — QC 채점

| 구분 | 첨부물 | 용도 |
|------|--------|------|
| **필수** | WF2 출력 HTML (또는 텍스트 변환본) | 채점 대상 |
| **필수** | GS2 보고서 전문 | 비교 기준 |
| **필수** | `system_prompt_quality_check_v2.md` | 채점 프롬프트 |
| 선택 | GS1 QC 결과 (PROGRESS_LOG 해당 세션) | GS1 vs GS2 점수 비교 |

#### Step 5-7 — 프롬프트 수정 설계 (조건부)

| 구분 | 첨부물 | 용도 |
|------|--------|------|
| **필수** | Step 5-6 QC 결과 (감점 항목 목록) | 수정 근거 |
| **필수** | 해당 섹션 프롬프트 (`section_XX.md`) | 수정 대상 |
| **필수** | GS2 해당 섹션 발췌 | 개선 기준 |
| **필수** | GS1 해당 섹션 발췌 | regression 고려 |
| 선택 | `system_prompt_quality_check_v2.md` | 채점 기준 참조 |

> **GS2 보고서 파일은 Step 5-2~5-7 전체에 걸쳐 핵심 첨부물 — 매번 첨부**
